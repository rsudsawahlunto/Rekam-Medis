...
Begin VB.Form FrmPeriodeLaporanBstatus 
   ...
   EndProperty
   Icon            =   "FrmPeriodeLaporanBStatus.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   930
      Left            =   0
      Picture         =   "FrmPeriodeLaporanBStatus.frx":08CA
      Top             =   0
      Width           =   10200
   End
End
Attribute VB_Name = "FrmPeriodeLaporanBstatus"
...
Dim intJmlTotal As Integer
    ...
    strSQL = "SELECT Ruangan,StatusPasien,SUM(JmlPasienPria) AS TJmlPasienPria, SUM(JmlPasienWanita) AS TJmlPasienWanita, SUM(Total) AS TTotal " & _
        " From V_RekapitulasiPasienBStatus " & _
        " WHERE (TglPendaftaran BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "') AND kdinstalasi = '" & dcInstalasi.BoundText & "'" & _
        " GROUP BY Ruangan, StatusPasien " & _
        " ORDER BY Ruangan, StatusPasien"
    ...
    For i = 1 To rs.RecordCount
        ...
        Next j
        rs.MoveNext
        'sub total per StatusPasien
        If rs.EOF = True Then GoTo stepSubTotalStatusPasien
        If rs("Ruangan").Value <> rsA("Ruangan").Value Then
stepSubTotalStatusPasien:
            intRowNow = intRowNow + 1
...
'    mdTglAkhir = dtpTglAkhir.Value
    strSQL = "SELECT * FROM V_RekapitulasiPasienBStatus " _
        & "WHERE (TglPendaftaran BETWEEN '" _
        & Format(DTPickerAwal, "yyyy/MM/dd 00:00:00") & "' AND '" _
        & Format(DTPickerAkhir, "yyyy/MM/dd 23:59:59") & "') ORDER BY Ruangan,statuspasien"
    Set rs = Nothing
    ...
    FrmViewerLaporan.Caption = "Medifirst2000 - Rekapitulasi Berdasarkan Status Pasien"
    cmdCetak.Enabled = True
...
Private Sub cmdgrafik_Click()
    ...
    cetak = "RekapstatusGrafik"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Grafik Rekapitulasi Berdasarkan Status Pasien"
    cmdCetak.Enabled = True
