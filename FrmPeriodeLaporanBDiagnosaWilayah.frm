...
Begin VB.Form FrmPeriodeLaporanBDiagnosaWilayah 
   ...
   EndProperty
   Icon            =   "FrmPeriodeLaporanBDiagnosaWilayah.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Image Image2 
      Height          =   930
      Left            =   0
      Picture         =   "FrmPeriodeLaporanBDiagnosaWilayah.frx":08CA
      Top             =   0
      Width           =   10200
   End
End
Attribute VB_Name = "FrmPeriodeLaporanBDiagnosaWilayah"
...
'    fgdata.Visible = False: MousePointer = vbHourglass
    ...
    strSQL = "SELECT Kecamatan, SUM(JmlPasienPria) AS JmlPria, SUM(JmlPasienWanita) AS JmlWanita, SUM(Total) AS Total" & _
        " FROM V_RekapitulasiDiagnosaPasienBWilayah " & _
        ...
        " GROUP BY Kecamatan" & _
        " ORDER BY Kecamatan"
    Call msubRecFO(rsA, strSQL)
    intJmlRow = rsA.RecordCount
    
    strSQL = "SELECT Kecamatan,Diagnosa,SUM(JmlPasienPria) AS TJmlPasienPria, SUM(JmlPasienWanita) AS TJmlPasienWanita, SUM(Total) AS TTotal " & _
        " From V_RekapitulasiDiagnosaPasienBWilayah " & _
        " WHERE (TglPeriksa BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "') AND kdinstalasi = '" & dcInstalasi.BoundText & "'" & _
        " GROUP BY Kecamatan, Diagnosa " & _
        " ORDER BY Kecamatan, Diagnosa"
    ...
    For i = 1 To rs.RecordCount
        ...
        If Trim(LCase(rs("Kecamatan").Value)) <> Trim(LCase(rsA("Kecamatan").Value)) Then
...
'            Debug.Print LCase(rs("Kecamatan").Value) & "   -    " & LCase(rsA("Kecamatan").Value)
...
'    mdTglAkhir = dtpTglAkhir.Value
    strSQL = "SELECT * FROM V_RekapitulasiDiagnosaPasienBWilayah " _
        & "WHERE (tglperiksa BETWEEN '" _
        & Format(DTPickerAwal, "yyyy/MM/dd 00:00:00") & "' AND '" _
        & Format(DTPickerAkhir, "yyyy/MM/dd 23:59:59") & "') ORDER BY kecamatan,diagnosa"
    Set rs = Nothing
    ...
    If rs.RecordCount = 0 Then
        ...
        Exit Sub
    End If
    cetak = "Rekapdiagnosawilayah"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Rekapitulasi Diagnosa Pasien Berdasarkan Wilayah"
    cmdCetak.Enabled = True
...
Private Sub cmdgrafik_Click()
    ...
    cetak = "RekapdiagnosawilayahGrafik"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Grafik Rekapitulasi Diagnosa Pasien Berdasarkan Wilayah"
    cmdCetak.Enabled = True
...
Private Sub subSetGrid()
    With fgdata
        ...
        .TextMatrix(0, 1) = "Wilayah"
        .TextMatrix(0, 2) = "Diagnosa"
