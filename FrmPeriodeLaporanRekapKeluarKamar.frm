...
Begin VB.Form FrmPeriodeLaporanRekapKeluarKamar 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Medifirst 2000 - Laporan Rekapitulasi Keluar Kamar Rawat Inap"
   ClientHeight    =   7710
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10230
   ...
   EndProperty
   Icon            =   "FrmPeriodeLaporanRekapKeluarKamar.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   930
      Left            =   0
      Picture         =   "FrmPeriodeLaporanRekapKeluarKamar.frx":08CA
      Top             =   0
      Width           =   10200
   End
End
Attribute VB_Name = "FrmPeriodeLaporanRekapKeluarKamar"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Dim iRowNow As Integer
Dim rsStatusKeluar As ADODB.recordset
...
Dim intJmlTotal As Integer
    ...
    strSQL = "SELECT Ruangan, SUM(JmlPasienPria) AS JmlPria, SUM(JmlPasienWanita) AS JmlWanita, SUM(Total) AS Total" & _
        " FROM V_RekapitulasiPasienRIKeluarKamar " & _
        " WHERE TglKeluar BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "' " & _
        " GROUP BY Ruangan" & _
        " ORDER BY Ruangan"
    Call msubRecFO(rsA, strSQL)
    intJmlRow = rsA.RecordCount
    
    If intJmlRow = 0 Then fgdata.Visible = True: MousePointer = vbDefault: DTPickerAwal.SetFocus: Exit Sub
    
    
    strSQL = "SELECT Ruangan,StatusKeluar,SUM(JmlPasienPria) AS TJmlPasienPria, SUM(JmlPasienWanita) AS TJmlPasienWanita, SUM(Total) AS TTotal " & _
        " From V_RekapitulasiPasienRIKeluarKamar " & _
        " WHERE (TglKeluar BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "') " & _
        " GROUP BY Ruangan, StatusKeluar " & _
        " ORDER BY Ruangan, StatusKeluar"
    ...
    For i = 1 To rs.RecordCount
        ...
        Next j
        rs.MoveNext
        'sub total per StatusKeluar
        If rs.EOF = True Then GoTo stepSubTotalStatusKeluar
        If rs("Ruangan").Value <> rsA("Ruangan").Value Then
stepSubTotalStatusKeluar:
            intRowNow = intRowNow + 1
...
'    mdTglAkhir = dtpTglAkhir.Value
    strSQL = "SELECT * FROM V_RekapitulasiPasienRIKeluarKamar " _
        & "WHERE (TglKeluar BETWEEN '" _
        & Format(DTPickerAwal, "yyyy/MM/dd 00:00:00") & "' AND '" _
        & Format(DTPickerAkhir, "yyyy/MM/dd 23:59:59") & "') ORDER BY Ruangan,StatusKeluar"
    Set rs = Nothing
    ...
    If rs.RecordCount = 0 Then
        ...
        Exit Sub
    End If
    cetak = "RekapKeluarKamarRI"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Rekapitulasi Keluar Kamar Rawat Inap"
    cmdCetak.Enabled = True
...
Private Sub cmdgrafik_Click()
    ...
    cetak = "RekapKeluarKamarRIGrafik"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Grafik Rekapitulasi Keluar Kamar Rawat Inap"
    cmdCetak.Enabled = True
