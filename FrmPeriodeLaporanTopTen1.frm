...
Begin VB.Form FrmPeriodeLaporanTopTen1 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Medifirst2000 - Rekapitulasi 10 Besar Penyakit"
   ClientHeight    =   8430
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12495
   ...
   EndProperty
   Icon            =   "FrmPeriodeLaporanTopTen1.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   945
      Left            =   8040
      Picture         =   "FrmPeriodeLaporanTopTen1.frx":0CCA
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1875
   End
   Begin VB.Image Image1 
      Height          =   975
      Left            =   1800
      Picture         =   "FrmPeriodeLaporanTopTen1.frx":1A52
      Stretch         =   -1  'True
      Top             =   0
      Width           =   13095
   End
   Begin VB.Image Image3 
      Height          =   975
      Left            =   0
      Picture         =   "FrmPeriodeLaporanTopTen1.frx":30B0
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1800
   End
End
Attribute VB_Name = "FrmPeriodeLaporanTopTen1"
...
Private Sub chkInstalasi_Click()
    ...
    If chkInstalasi.value = vbChecked Then
        ...
        Call msubDcSource(dcInstalasi, rs, "SELECT KdInstalasi, NamaInstalasi FROM Instalasi WHERE (KdInstalasi IN ('01', '02', '03', '06', '08')) and StatusEnabled='1'")
...
Private Sub chRuangPoli_Click()
    ...
    If chRuangPoli.value = vbChecked Then
        ...
        Call msubDcSource(dcRuangPoli, rs, "SELECT KdRuangan, NamaRuangan FROM Ruangan WHERE (KdInstalasi = '" & dcInstalasi.BoundText & "') and StatusEnabled='1' order by NamaRuangan")
        dcRuangPoli.Text = rs(1).value
    Else
        dcRuangPoli.Enabled = False
        dcRuangPoli.Text = ""
        optKodeDiagnosa.SetFocus
    End If
    Exit Sub
hell:
    Call msubPesanError
...
Private Sub Cek(SekalianCetak As Boolean, Biasa As Boolean)
    ...
    If chkInstalasi.value = vbChecked Then
        ...
        mstrFilter2 = dcInstalasi.BoundText
    Else
...
'        strSQL = "SELECT top " & Val(txtJmlData) & " KdDiagnosa, Diagnosa, sum(jumlahpasien) as [JmlPasien]" & _
...
'        " JenisPasien LIKE '%" & dcJenisPasien & "%' group by KdDiagnosa, Diagnosa order by Diagnosa asc"
'    Else
'        strSQL = "SELECT top " & Val(txtJmlData) & " KdDiagnosa, Diagnosa, sum(jumlahpasien) as [JmlPasien]" & _
'        " FROM V_RekapitulasiDiagnosaTopTen " & _
'        " WHERE TglPeriksa BETWEEN " & _
...
'        " '" & Format(DTPickerAkhir.value, "yyyy/MM/dd 23:59:59") & "' " & _
'        " AND kdinstalasi " & mstrFilter & " AND KdRuangan LIKE '%" & dcRuangPoli.BoundText & "%' AND " & _
'        " JenisPasien LIKE '%" & dcJenisPasien & "%' group by KdDiagnosa, Diagnosa order by [JmlPasien] desc"
'    End If

    If optKodeDiagnosa.value = True Then
        strSQL = "SELECT top " & Val(txtJmlData) & " KdDiagnosa, Diagnosa," & _
        " dbo.JmlPasienPerDiagnosa('" & Format(DTPickerAwal.value, "yyyy/MM/dd 00:00:00") & "','" & Format(DTPickerAkhir.value, "yyyy/MM/dd 23:59:59") & "',KdDiagnosa,'" & mstrFilter2 & "') AS [JmlPasien], sum(jumlahpasien) as [JmlKunjungan] " & _
    ...
    Else
        If SekalianCetak = True Then
            If Biasa = True Then
                ...
                cetak = "RekapTopten"
                FrmViewerLaporan1.Show
                FrmViewerLaporan1.Caption = "Medifirst2000 - Rekapitulasi Penyakit"
            ElseIf Biasa = False Then
                vLaporan = ""
                If MsgBox("Apakah Anda Ingin Langsung Mencetak Laporan?" & vbNewLine & "Pilih No Jika Ingin Ditampilkan Terlebih Dahulu", vbYesNo, "Medifirst2000 - Cetak Laporan") = vbNo Then vLaporan = "view"
                cetak = "RekapToptenGrafik"
                FrmViewerLaporan1.Show
                FrmViewerLaporan1.Caption = "Medifirst2000 - Grafik Rekapitulasi Penyakit"
            End If
        End If
...
Private Sub chRuangPoli_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        If chRuangPoli.value = vbChecked Then
            dcRuangPoli.Enabled = True
            Call msubDcSource(dcRuangPoli, rs, "SELECT KdRuangan, NamaRuangan FROM Ruangan WHERE (KdInstalasi = '" & dcInstalasi.BoundText & "') and StatusEnabled='1'")
            dcRuangPoli.Text = rs(1).value
...
Private Sub dcInstalasi_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        ...
        strSQL = "SELECT KdInstalasi, NamaInstalasi FROM Instalasi WHERE (KdInstalasi IN ('01', '02', '03', '06', '08')) and StatusEnabled='1' and (NamaInstalasi LIKE '%" & dcInstalasi.Text & "%')"
...
Private Sub dcRuangPoli_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        If dcRuangPoli.MatchedWithList = True Then optKodeDiagnosa.SetFocus
        strSQL = "SELECT KdRuangan, NamaRuangan FROM Ruangan WHERE (KdInstalasi = '" & dcInstalasi.BoundText & "') and StatusEnabled='1' and (NamaRuangan LIKE '%" & dcRuangPoli.Text & "%')"
        Call msubRecFO(rs, strSQL)
        If rs.EOF = True Then
            dcRuangPoli.Text = ""
            Exit Sub
