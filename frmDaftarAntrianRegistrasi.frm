...
Public Sub cmdCari_Click()
    ...
    If dtpAwal.Day <> dtpAkhir.Day Or dtpAwal.Month <> dtpAkhir.Month Or dtpAwal.Year <> dtpAkhir.Year Then
        ...
        " where ( NoPendaftaran = '0000000000' or NoPendaftaran is null) and TglAntrian between '" & Format(dtpAwal.value, "yyyy/MM/dd HH:mm:00") & "' and '" & Format(dtpAkhir.value, "yyyy/MM/dd HH:mm:59") & "' and JenisPasien='" & cbLoket.Text & "'" & _
        " order by status"
    Else
        strSQL = "select TOP 100 * " & _
        " from V_AntrianPasienRegistrasi " & _
...
'        End If
        ...
        With frmRegistrasiAll
                ...
                strSQL1 = "SELECT NamaLengkap,JenisKelamin FROM Pasien WHERE NoCM='" & dgDaftarAntrianPasien.Columns(3).value & "'"
                Set rs1 = Nothing
                Call msubRecFO(rs1, strSQL1)
                .txtNamaPasien.Text = rs1(0).value
                If rs1(1) = "L" Then
