...
Begin VB.Form FrmPeriodeLaporanBWilayah 
   ...
   EndProperty
   Icon            =   "FrmPeriodeLaporanBWilayah.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   930
      Left            =   0
      Picture         =   "FrmPeriodeLaporanBWilayah.frx":08CA
      Top             =   0
      Width           =   10200
   End
End
Attribute VB_Name = "FrmPeriodeLaporanBWilayah"
...
Dim intJmlTotal As Integer
    ...
    strSQL = "SELECT [Ruang Pemeriksaan], SUM(JmlPasienPria) AS JmlPria, SUM(JmlPasienWanita) AS JmlWanita, SUM(Total) AS Total" & _
        " FROM V_RekapitulasiPasienBWilayah " & _
        ...
        " GROUP BY [Ruang Pemeriksaan]" & _
        " ORDER BY [Ruang Pemeriksaan]"
    Call msubRecFO(rsA, strSQL)
    intJmlRow = rsA.RecordCount
    
    strSQL = "SELECT [Ruang Pemeriksaan],Kecamatan,SUM(JmlPasienPria) AS TJmlPasienPria, SUM(JmlPasienWanita) AS TJmlPasienWanita, SUM(Total) AS TTotal " & _
        " From V_RekapitulasiPasienBWilayah " & _
        " WHERE (TglPendaftaran BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "') AND kdinstalasi = '" & dcInstalasi.BoundText & "'" & _
        " GROUP BY [Ruang Pemeriksaan], Kecamatan " & _
        " ORDER BY [Ruang Pemeriksaan], Kecamatan"
    ...
    For i = 1 To rs.RecordCount
        ...
        If rs.EOF = True Then GoTo stepSubTotalKecamatan
        If rs("Ruang Pemeriksaan").Value <> rsA("Ruang Pemeriksaan").Value Then
stepSubTotalKecamatan:
            intRowNow = intRowNow + 1
...
'    mdTglAkhir = dtpTglAkhir.Value
    strSQL = "SELECT * FROM V_RekapitulasiPasienBWilayah " _
        & "WHERE (TglPendaftaran BETWEEN '" _
        & Format(DTPickerAwal, "yyyy/MM/dd 00:00:00") & "' AND '" _
        & Format(DTPickerAkhir, "yyyy/MM/dd 23:59:59") & "') ORDER BY [Ruang Pemeriksaan],kecamatan"
    Set rs = Nothing
    ...
    FrmViewerLaporan.Caption = "Medifirst2000 - Rekapitulasi Berdasarkan Wilayah Pasien"
    cmdCetak.Enabled = True
...
Private Sub cmdgrafik_Click()
    ...
    cetak = "RekapwilayahGrafik"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Grafik Rekapitulasi Berdasarkan Wilayah Pasien"
    cmdCetak.Enabled = True
