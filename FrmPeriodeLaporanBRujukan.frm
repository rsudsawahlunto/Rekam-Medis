...
Begin VB.Form FrmPeriodeLaporanBRujukan 
   ...
   EndProperty
   Icon            =   "FrmPeriodeLaporanBRujukan.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   930
      Left            =   0
      Picture         =   "FrmPeriodeLaporanBRujukan.frx":08CA
      Top             =   0
      Width           =   10200
   End
End
Attribute VB_Name = "FrmPeriodeLaporanBRujukan"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Dim iRowNow As Integer
Dim rsrujukanasal As ADODB.recordset
...
Dim intJmlTotal As Integer
    ...
    strSQL = "SELECT Ruangan, SUM(JmlPasienPria) AS JmlPria, SUM(JmlPasienWanita) AS JmlWanita, SUM(Total) AS Total" & _
        " FROM V_RekapitulasiPasienBRujukan " & _
    ...
    strSQL = "SELECT Ruangan,RujukanAsal,SUM(JmlPasienPria) AS TJmlPasienPria, SUM(JmlPasienWanita) AS TJmlPasienWanita, SUM(Total) AS TTotal " & _
        " From V_RekapitulasiPasienBRujukan " & _
        " WHERE (TglPendaftaran BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "') AND kdinstalasi = '" & dcInstalasi.BoundText & "'" & _
        " GROUP BY Ruangan, RujukanAsal " & _
        " ORDER BY Ruangan, RujukanAsal"
    ...
    For i = 1 To rs.RecordCount
        ...
        Next j
        rs.MoveNext
        'sub total per RujukanAsal
        If rs.EOF = True Then GoTo stepSubTotalRujukanAsal
        If rs("Ruangan").Value <> rsA("Ruangan").Value Then
stepSubTotalRujukanAsal:
            intRowNow = intRowNow + 1
...
'    mdTglAkhir = dtpTglAkhir.Value
    strSQL = "SELECT * FROM V_RekapitulasiPasienBRujukan " _
        & "WHERE (TglPendaftaran BETWEEN '" _
        & Format(DTPickerAwal, "yyyy/MM/dd 00:00:00") & "' AND '" _
        & Format(DTPickerAkhir, "yyyy/MM/dd 23:59:59") & "') ORDER BY Ruangan,RujukanAsal"
    Set rs = Nothing
    ...
    FrmViewerLaporan.Caption = "Medifirst2000 - Rekapitulasi Berdasarkan Rujukan Pasien"
    cmdCetak.Enabled = True
...
Private Sub cmdgrafik_Click()
    ...
    cetak = "RekaprujukanGrafik"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Grafik Rekapitulasi Berdasarkan Rujukan Pasien"
    cmdCetak.Enabled = True
