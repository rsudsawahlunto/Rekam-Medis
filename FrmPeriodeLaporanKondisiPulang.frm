...
Dim intJmlTotal As Integer
    ...
    strSQL = "SELECT NamaRuangan, SUM(JmlPasienPria) AS JmlPria, SUM(JmlPasienWanita) AS JmlWanita, SUM(Total) AS Total" & _
        ...
        " WHERE TglKeluar BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "' AND kdinstalasi = '" & dcInstalasi.BoundText & "'" & _
        " GROUP BY NamaRuangan" & _
        " ORDER BY NamaRuangan"
    Call msubRecFO(rsA, strSQL)
    intJmlRow = rsA.RecordCount
    
    strSQL = "SELECT NamaRuangan,KondisiPulang,SUM(JmlPasienPria) AS TJmlPasienPria, SUM(JmlPasienWanita) AS TJmlPasienWanita, SUM(Total) AS TTotal " & _
        " From V_RekapitulasiBKondisiPulang " & _
        " WHERE (TglKeluar BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "') AND kdinstalasi = '" & dcInstalasi.BoundText & "'" & _
        " GROUP BY NamaRuangan, KondisiPulang " & _
        " ORDER BY NamaRuangan, KondisiPulang"
    ...
    For i = 1 To rs.RecordCount
        ...
        Next j
        rs.MoveNext
        'sub total per KondisiPulang
        If rs.EOF = True Then GoTo stepSubTotalKondisiPulang
        If rs("NamaRuangan").Value <> rsA("NamaRuangan").Value Then
stepSubTotalKondisiPulang:
            intRowNow = intRowNow + 1
