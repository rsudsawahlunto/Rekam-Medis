...
Begin VB.Form FrmPeriodeLaporanJenisOperasiIBS 
   ...
   EndProperty
   Icon            =   "FrmPeriodeLaporanJenisOperasiIBS.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   930
      Left            =   0
      Picture         =   "FrmPeriodeLaporanJenisOperasiIBS.frx":08CA
      Top             =   0
      Width           =   10200
   End
End
Attribute VB_Name = "FrmPeriodeLaporanJenisOperasiIBS"
...
Dim intJmlTotal As Integer
    ...
    strSQL = "SELECT NamaRuangan, SUM(JmlPasienPria) AS JmlPria, SUM(JmlPasienWanita) AS JmlWanita, SUM(Total) AS Total" & _
        " FROM V_RekapitulasiJenisOperasiIBS " & _
        ...
        " GROUP BY NamaRuangan" & _
        " ORDER BY NamaRuangan"
    Call msubRecFO(rsA, strSQL)
    intJmlRow = rsA.RecordCount
    
    strSQL = "SELECT NamaRuangan,JenisOperasi,SUM(JmlPasienPria) AS TJmlPasienPria, SUM(JmlPasienWanita) AS TJmlPasienWanita, SUM(Total) AS TTotal " & _
        " From V_RekapitulasiJenisOperasiIBS " & _
        " WHERE (TglPendaftaran BETWEEN '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "') AND kdinstalasi = '" & dcInstalasi.BoundText & "'" & _
        " GROUP BY NamaRuangan, JenisOperasi " & _
        " ORDER BY NamaRuangan, JenisOperasi"
    ...
    For i = 1 To rs.RecordCount
        ...
        Next j
        rs.MoveNext
        'sub total per JenisOperasi
        If rs.EOF = True Then GoTo stepSubTotalJenisOperasi
        If rs("NamaRuangan").Value <> rsA("NamaRuangan").Value Then
stepSubTotalJenisOperasi:
            intRowNow = intRowNow + 1
...
'    mdTglAkhir = dtpTglAkhir.Value
    strSQL = "SELECT * FROM V_RekapitulasiJenisOperasiIBS " _
        & "WHERE (TglPendaftaran BETWEEN '" _
        & Format(DTPickerAwal, "yyyy/MM/dd 00:00:00") & "' AND '" _
        & Format(DTPickerAkhir, "yyyy/MM/dd 23:59:59") & "') ORDER BY namaruangan,jenisoperasi"
    Set rs = Nothing
    ...
    If rs.RecordCount = 0 Then
        ...
        Exit Sub
    End If
    cetak = "RekapJenisOperasi"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Rekapitulasi Jenis Operasi Pasien"
    cmdCetak.Enabled = True
...
Private Sub cmdgrafik_Click()
    ...
    cetak = "Rekapgrafikjenisoperasi"
    FrmViewerLaporan.Show
    FrmViewerLaporan.Caption = "Medifirst2000 - Grafik Rekapitulasi Jenis Operasi Pasien"
    cmdCetak.Enabled = True
