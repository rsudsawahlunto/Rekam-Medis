...
Begin VB.Form frmPasienBaru2 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Medifirst2000 - Detail Pasien"
   ClientHeight    =   9390
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8805
   ...
   Begin VB.Frame fraPasien 
      ...
      Begin VB.ComboBox cboNamaDepan 
         ...
         EndProperty
         Height          =   390
         ItemData        =   "frmPasienBaru2.frx":0CCA
         Left            =   240
         List            =   "frmPasienBaru2.frx":0CDD
         Style           =   2  'Dropdown List
         TabIndex        =   4
         Top             =   600
         Width           =   1335
      ...
      Begin VB.ComboBox cboJnsKelaminPasien 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "frmPasienBaru2.frx":0CFB
         Left            =   240
         List            =   "frmPasienBaru2.frx":0D05
         Style           =   2  'Dropdown List
         TabIndex        =   10
         Top             =   1320
         Width           =   1335
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cboPegawaiToPasien 
         ...
         Enabled         =   0   'False
         Height          =   330
         ItemData        =   "frmPasienBaru2.frx":0D1F
         Left            =   5640
         List            =   "frmPasienBaru2.frx":0D21
         Style           =   2  'Dropdown List
         TabIndex        =   3
         Top             =   480
         Visible         =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   945
      Left            =   6960
      Picture         =   "frmPasienBaru2.frx":0D23
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1875
   End
   Begin VB.Image Image1 
      Height          =   975
      Left            =   1800
      Picture         =   "frmPasienBaru2.frx":1AAB
      Stretch         =   -1  'True
      Top             =   0
      Width           =   13095
   End
   Begin VB.Image Image3 
      Height          =   975
      Left            =   0
      Picture         =   "frmPasienBaru2.frx":3109
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1800
   End
End
Attribute VB_Name = "frmPasienBaru2"
...
Private Sub subLoadBayi()
    On Error GoTo hell_
    Set rs = Nothing
    strSQL = "SELECT  NamaIbuBayi, JKBayi, TglLahir, NoUrutBayiLahir,Tahun,Bulan,Hari From V_DaftarNamaIbuBersalin " & _
...
Private Sub txtNoCM_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        strSQL = "SELECT NoCM, Title || ' ' || + NamaLengkap AS NamaPasien FROM V_CariPasien WHERE (NoCM = '" & txtNoCM.Text & "' )"
        Call msubRecFO(rs, strSQL)
...
''        .Parameters.Append .CreateParameter("NoCM", adChar, adParamInput, 6, IIf(Trim(txtNoCM.Text) = "", Null, Trim(txtNoCM.Text)))
...
''        End If
''        .Parameters.Append .CreateParameter("TglDaftarMembership", adDBTimeStamp, adParamInput, , Format(dtpTglPendaftaran.value, "yyyy-MM-dd HH:mm:ss"))
...
''        End If
''        .Parameters.Append .CreateParameter("TglLahir", adDBTimeStamp, adParamInput, , Format(meTglLahir.Text, "yyyy-MM-dd HH:mm:ss"))
...
''        End If
''        .Parameters.Append .CreateParameter("OutputNoCM", adChar, adParamInput, 6, Null)
...
''            If Not IsNull(rsCmd(0)) Then mstrNoCM = rsCmd(0)
...
Private Sub subLoadDataWilayah(strPencarian As String)
    ...
    strSQL = "SELECT DISTINCT nvl(NamaPropinsi, '') AS NamaPropinsi, nvl(NamaKotaKabupaten, '') AS NamaKotaKabupaten, nvl(NamaKecamatan, '')  AS NamaKecamatan, nvl(NamaKelurahan, '') AS NamaKelurahan, nvl(KodePos, '') AS KodePos" & _
    " FROM V_Wilayah" & _
    " " & strTempSql
...
Private Sub subPrintRegistrasiBarcode()
    ...
    mstrServerPrinterBarcode = GetStringValue("HKEY_CURRENT_USER\Software\Medifirst2000_Informix\Standard", "ServerPrinterBarcode")
